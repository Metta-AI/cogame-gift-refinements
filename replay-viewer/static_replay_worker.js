...
var Module = {};
// Gift Refinements records STATE, not inputs, so playback never re-simulates
// and there is no replay hash to mismatch: the ctf_mismatch_tick export and
// every message field that carried it are gone.
...
function stageNote() {
  ...
  try {
    var length = Module._gr_stage_len ? Module._gr_stage_len() : 0;
    if (!length) return '';
    var pointer = Module._gr_stage_ptr();
...
function runtimeError() {
  var length = Module._gr_error_len();
  if (!length) {
    var stage = stageNote();
    return stage
  ...
  var pointer = Module._gr_error_ptr();
  return new TextDecoder().decode(
    Module.HEAPU8.slice(pointer, pointer + length));
}
...
function ingestPacket() {
  var length = Module._gr_packet_len();
  if (!length) throw new Error('Replay runtime produced an empty frame');
  var pointer = Module._gr_packet_ptr();
...
function sendRuntimeInput(bytes) {
  ...
  copyIntoRuntime(bytes, function (pointer, length) {
    Module._gr_input(pointer, length);
  });
}
...
async function start() {
  ...
  try {
    ...
    var loaded = copyIntoRuntime(bytes, function (pointer, length) {
      return Module._gr_load_replay(pointer, length);
    });
...
function applyInputNow() {
  ...
  try {
    if (Module._gr_frame() < 0) throw new Error(runtimeError());
    ingestPacket();
...
importScripts('./wire_constants.js', './broadcast_core.js', './gift_refinements_replay.js');
